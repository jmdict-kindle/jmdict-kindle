  ...
  def __init__(self):
    ...
    with open("./pronunciation/ACCDB_unicode.csv", encoding="utf-8") as file:
      ...
      for row in csv_reader:
        self.dict[f"{row['kanjiexpr']}-{row['midashigo'].translate(translate_table)}"] = {
    ...
    with open("./pronunciation/accents.tsv", encoding="utf-8") as file:
      csv_reader = csv.DictReader(file, delimiter='\t', fieldnames=['kanjiexpr', 'midashigo', 'accent'])
      for row in csv_reader:
        if row['midashigo'] == '':
          row['midashigo'] = row['kanjiexpr'] #if only kana then there is no midashigo
          
        accents = row['accent'].split(',')#there can be multiple pronunciations per word

        #find best accent based on priority
        used_accent = ''
        priority = -1
        for accent in accents:
          if '(' in accent: #some accents contain info about when to use. Do not use elif since the same accen position can have multiple qualifiers
            ...
            if '感' in accent and priority < 20: #interjection
              priority = 20
              used_accent = accent
            if not ('名' in accent or '副' in accent or '代' in accent or '形動' in accent or '感' in accent):
        ...
        if priority == -1:
          raise Exception(f"no accent selected for {row['kanjiexpr']} {row['midashigo']} {row['accent']}")

        #Turn accent position into notation used by ACCDB_unicode.csv
        accent_position_sound = int(''.join((ch if ch in '0123456789' else '') for ch in used_accent)) #get only number from string.
        ...
        if accent_position_sound > kana_count:
          sys.stdout.write(f"the accent position for {row['midashigo']} is greater than the string length ({accent_position_sound})\n")
          continue
        #The accent position in the file looks at a word per sound, however some sounds consist of two kana e.g. しゃ
        #correct for that here
        i = 0
        sound_changer = u"ぁぃぅぇぉゃゅょァィゥェォャュョ" # っ isn't in there by design as it is considered a separate sound by accents.tsv.
        ...
        if kana_count > falling_accent_position:
          if row['midashigo'][falling_accent_position] in sound_changer or row['midashigo'][falling_accent_position] == "ー":#if position after falling_accent_position is as small kana. Not +1 because array starts at 0
            falling_accent_position += 1
        ...
        if accent_position_sound == 0:
          ac = \
            ...
            ''.join('1' for i in range(first_sound_character_count + 1, kana_count + 1))
        elif accent_position_sound == 1:
          ac = \
            ''.join('1' for i in range(1, first_sound_character_count))+\
            "2"+\
            ''.join('0' for i in range(first_sound_character_count + 1, kana_count + 1))
        else:
          ac = \
            ''.join('0' for i in range(1, first_sound_character_count + 1))+\
            ''.join('1' for i in range(first_sound_character_count + 1, falling_accent_position))+\
            "2"+\
